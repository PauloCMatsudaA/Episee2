...
- Suas respostas serão enviadas via Telegram; evite markdown pesado,
...
def _epi_relevante(epi: EpiType, mensagem_lower: str) -> bool:
    """
    Verifica se o EPI é relevante para a mensagem usando:
    1. Nome exato do EPI
    2. Cada palavra-chave cadastrada pelo gestor (separadas por vírgula)
    """
    # 1. Nome do EPI
    if epi.nome.lower() in mensagem_lower:
        return True

    # 2. Palavras-chave / sinônimos
...
async def _buscar_contexto_epi(mensagem: str) -> str:
    """
    RAG: busca EPIs relevantes pelo nome ou por palavras-chave cadastradas
    pelo gestor e monta o bloco de contexto injetado no prompt do DeepSeek.
    """
    try:
        ...
        for epi in epis:
            ...
            # Vídeos aprovados (máx. 3, por prioridade)
...
async def transcrever_audio_telegram(file_id: str) -> str:
    """Baixa áudio do Telegram e transcreve via DeepSeek (compatível Whisper)."""
    try:
